import { Direction, } from './typings.js';
class Snake {
    body;
    snake = [[0, 0], [0, 1], [0, 2]]; // [...[y,x]]
    ...
    getSnakeHandAndLength() {
        const snakeLength = this.snake.length, [snakeHead, ...snakeBody] = [...this.snake].reverse();
        return {
            snakeHead,
            snakeBody,
            snakeLength,
    ...
    eatFood() {
        ...
        if (snakeHead.toString() !== this.food.toString()) {
            return false;
        }
        const snakeItem = [...this.snake[0]];
    ...
    createRandomPotion() {
        ...
        let randomPostion = [Math.floor(Math.random() * 10), Math.floor(Math.random() * 10)];
